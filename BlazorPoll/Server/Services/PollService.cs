...
namespace BlazorPoll.Server.Services
{
    public class PollService
    {
		...
		public Poll GetPoll(Guid id)
		{
			...
			pollEntity.PollOptions.OrderBy(x => x.Votes);
		...
		{
			var currentPolls = applicationDbContext.Polls.Where(x => x.PollEnd > DateTime.UtcNow).OrderByDescending(x => x.PollEnd).ToList();
